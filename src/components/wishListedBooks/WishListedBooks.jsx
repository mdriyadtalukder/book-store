...
import { getAllOption2, getOption2, getSearch2, setPage, setPerPage, setTotalEntries } from "../../rtk-query/features/books/booksSlice";
import { useEffect } from "react";
import { CiSearch } from "react-icons/ci";

const WishListedBooks = () => {
    const { currentPage, perPage, wishlist, search2, option2, allOptions2 } = useSelector((state) => state.books);
    const dispatch = useDispatch();
    const [searchParams] = useSearchParams();

    useEffect(() => {
        if (wishlist) {
            const uniqueBValues = [...new Set(wishlist.flatMap(item => item.subjects))];
            dispatch(getAllOption2(uniqueBValues));
        }
        ...
        <div className="max-w-screen mx-auto xl:px-20 md:px-10 sm:px-2 px-4 pt-20 overflow-hidden">
            <div className="  pt-4 flex sm:flex-row flex-col sm:gap-10 gap-3 justify-center items-center">
                <select onChange={(e) => dispatch(getOption2(e.target.value))} className="flex-1 max-w-44   outline-none focus:outline-none border-none p-2 rounded-lg ">
                    <option selected={option2 == "All genre"} className="">All genre</option>
                    {allOptions2?.map((value, idx) => (
                        <option disabled={!entries?.some(book => book.subjects.includes(value))} selected={option2 === value} key={idx} value={value}>{value}</option>
                    ))}
                </select>
                <div className="relative">
                    <input defaultValue={search2} onChange={(e) => dispatch(getSearch2(e.target.value))} placeholder="Search by title..." type="text" className=" focus:outline-none p-1 rounded-full text-black " />
